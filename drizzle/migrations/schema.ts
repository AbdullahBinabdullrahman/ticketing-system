import { pgTable, unique, serial, varchar, text, timestamp, integer, boolean, type AnyPgColumn, index, foreignKey, numeric, check, jsonb, pgEnum } from "drizzle-orm/pg-core"
import { sql } from "drizzle-orm"

export const assignmentResponseEnum = pgEnum("assignment_response_enum", ['confirmed', 'rejected', 'timeout', 'pending'])
export const branchUserRoleEnum = pgEnum("branch_user_role_enum", ['branch_manager', 'technician', 'viewer', 'admin'])
export const configScopeEnum = pgEnum("config_scope_enum", ['global', 'partner'])
export const notificationTypeEnum = pgEnum("notification_type_enum", ['request_submitted', 'request_assigned', 'request_confirmed', 'request_rejected', 'request_in_progress', 'request_completed', 'request_closed', 'partner_timeout', 'sla_reminder', 'general'])
export const partnerStatusEnum = pgEnum("partner_status_enum", ['active', 'inactive', 'suspended'])
export const requestStatusEnum = pgEnum("request_status_enum", ['submitted', 'assigned', 'confirmed', 'in_progress', 'completed', 'closed', 'rejected', 'unassigned'])
export const userTypeEnum = pgEnum("user_type_enum", ['admin', 'partner', 'customer'])
...
export const users = pgTable("users", {
	...
	languagePreference: varchar("language_preference", { length: 5 }).default('en'),
	...
	index("idx_users_email").using("btree", table.email.asc().nullsLast().op("text_ops")).where(sql`(is_deleted = false)`),
	index("idx_users_partner_id").using("btree", table.partnerId.asc().nullsLast().op("int4_ops")).where(sql`(partner_id IS NOT NULL)`),
	index("idx_users_role_id").using("btree", table.roleId.asc().nullsLast().op("int4_ops")),
	index("idx_users_user_type").using("btree", table.userType.asc().nullsLast().op("enum_ops")),
	foreignKey({
...
export const userPermissions = pgTable("user_permissions", {
	...
	unique("user_permissions_user_id_permission_id_key").on(table.userId, table.permissionId),
]);
...
export const customers = pgTable("customers", {
	...
	index("idx_customers_user_id").using("btree", table.userId.asc().nullsLast().op("int4_ops")),
	foreignKey({
			columns: [table.userId],
			foreignColumns: [users.id],
...
export const partners = pgTable("partners", {
	...
	index("idx_partners_status").using("btree", table.status.asc().nullsLast().op("enum_ops")).where(sql`(is_deleted = false)`),
	foreignKey({
			columns: [table.createdById],
			foreignColumns: [users.id],
...
export const branches = pgTable("branches", {
	...
	index("idx_branches_lat_lng").using("btree", table.lat.asc().nullsLast().op("numeric_ops"), table.lng.asc().nullsLast().op("numeric_ops")),
	index("idx_branches_partner_id").using("btree", table.partnerId.asc().nullsLast().op("int4_ops")).where(sql`(is_deleted = false)`),
	foreignKey({
			columns: [table.createdById],
			foreignColumns: [users.id],
...
export const branchUsers = pgTable("branch_users", {
	...
	index("idx_branch_users_branch_id").using("btree", table.branchId.asc().nullsLast().op("int4_ops")).where(sql`(is_deleted = false)`),
	index("idx_branch_users_user_id").using("btree", table.userId.asc().nullsLast().op("int4_ops")).where(sql`(is_deleted = false)`),
	foreignKey({
	...
	unique("branch_users_branch_id_user_id_key").on(table.branchId, table.userId),
]);
...
export const categories = pgTable("categories", {
	...
	index("idx_categories_active").using("btree", table.isActive.asc().nullsLast().op("bool_ops")).where(sql`(is_deleted = false)`),
	foreignKey({
			columns: [table.createdById],
			foreignColumns: [users.id],
...
export const partnerCategories = pgTable("partner_categories", {
	...
	unique("partner_categories_partner_id_category_id_key").on(table.partnerId, table.categoryId),
]);
...
export const services = pgTable("services", {
	...
	index("idx_services_category_id").using("btree", table.categoryId.asc().nullsLast().op("int4_ops")).where(sql`(is_deleted = false)`),
	foreignKey({
			columns: [table.categoryId],
			foreignColumns: [categories.id],
...
export const pickupOptionTypes = pgTable("pickup_option_types", {
	...
	description: text(),
	descriptionAr: text("description_ar"),
	requiresServiceSelection: boolean("requires_service_selection").default(false),
	createdAt: timestamp("created_at", { withTimezone: true, mode: 'string' }).default(sql`CURRENT_TIMESTAMP`),
	createdById: integer("created_by_id"),
	updatedAt: timestamp("updated_at", { withTimezone: true, mode: 'string' }).default(sql`CURRENT_TIMESTAMP`),
	updatedById: integer("updated_by_id"),
...
export const partnerPickupOptions = pgTable("partner_pickup_options", {
	id: serial().primaryKey().notNull(),
	partnerId: integer("partner_id").notNull(),
	...
	unique("partner_pickup_options_partner_id_pickup_option_type_id_key").on(table.partnerId, table.pickupOptionTypeId),
]);
...
export const requests = pgTable("requests", {
	...
	submittedAt: timestamp("submitted_at", { withTimezone: true, mode: 'string' }).default(sql`CURRENT_TIMESTAMP`),
	...
	index("idx_requests_branch_id").using("btree", table.branchId.asc().nullsLast().op("int4_ops")),
	index("idx_requests_category_id").using("btree", table.categoryId.asc().nullsLast().op("int4_ops")),
	index("idx_requests_created_at").using("btree", table.createdAt.asc().nullsLast().op("timestamptz_ops")),
	index("idx_requests_customer_id").using("btree", table.customerId.asc().nullsLast().op("int4_ops")),
	index("idx_requests_metadata").using("gin", table.metadata.asc().nullsLast().op("jsonb_ops")),
	index("idx_requests_partner_id").using("btree", table.partnerId.asc().nullsLast().op("int4_ops")),
	index("idx_requests_request_number").using("btree", table.requestNumber.asc().nullsLast().op("text_ops")),
	index("idx_requests_sla_deadline").using("btree", table.slaDeadline.asc().nullsLast().op("timestamptz_ops")).where(sql`(status = 'assigned'::request_status_enum)`),
	index("idx_requests_status").using("btree", table.status.asc().nullsLast().op("enum_ops")),
	index("idx_requests_submitted_at").using("btree", table.submittedAt.asc().nullsLast().op("timestamptz_ops")),
	foreignKey({
...
export const requestAssignments = pgTable("request_assignments", {
	...
	index("idx_request_assignments_partner_id").using("btree", table.partnerId.asc().nullsLast().op("int4_ops")),
	index("idx_request_assignments_request_id").using("btree", table.requestId.asc().nullsLast().op("int4_ops")),
	index("idx_request_assignments_response").using("btree", table.response.asc().nullsLast().op("enum_ops")),
	foreignKey({
			columns: [table.assignedByUserId],
			foreignColumns: [users.id],
...
export const requestStatusLog = pgTable("request_status_log", {
	...
	index("idx_request_status_log_request_id").using("btree", table.requestId.asc().nullsLast().op("int4_ops")),
	index("idx_request_status_log_timestamp").using("btree", table.timestamp.asc().nullsLast().op("timestamptz_ops")),
	foreignKey({
...
export const notifications = pgTable("notifications", {
	...
	index("idx_notifications_read").using("btree", table.read.asc().nullsLast().op("bool_ops"), table.userId.asc().nullsLast().op("int4_ops")).where(sql`(is_deleted = false)`),
	index("idx_notifications_request_id").using("btree", table.requestId.asc().nullsLast().op("int4_ops")).where(sql`(request_id IS NOT NULL)`),
	index("idx_notifications_user_id").using("btree", table.userId.asc().nullsLast().op("int4_ops")).where(sql`(is_deleted = false)`),
	foreignKey({
			columns: [table.createdById],
			foreignColumns: [users.id],
...
export const configurations = pgTable("configurations", {
	...
	index("idx_configurations_partner_id").using("btree", table.partnerId.asc().nullsLast().op("int4_ops")).where(sql`(partner_id IS NOT NULL)`),
	index("idx_configurations_scope").using("btree", table.scope.asc().nullsLast().op("enum_ops")).where(sql`(is_deleted = false)`),
	foreignKey({
			columns: [table.createdById],
			foreignColumns: [users.id],
	...
	check("partner_id_required_for_partner_scope", sql`((scope = 'partner'::config_scope_enum) AND (partner_id IS NOT NULL)) OR ((scope = 'global'::config_scope_enum) AND (partner_id IS NULL))`),
]);
...
export const logs = pgTable("logs", {
	...
	index("idx_logs_actor_id").using("btree", table.actorId.asc().nullsLast().op("int4_ops")),
	index("idx_logs_entity_type_id").using("btree", table.entityType.asc().nullsLast().op("int4_ops"), table.entityId.asc().nullsLast().op("text_ops")),
	index("idx_logs_metadata").using("gin", table.metadata.asc().nullsLast().op("jsonb_ops")),
	index("idx_logs_timestamp").using("btree", table.timestamp.asc().nullsLast().op("timestamptz_ops")),
	foreignKey({
