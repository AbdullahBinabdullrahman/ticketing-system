    ...
    try {
      ...
      for (const request of expiredRequests) {
        try {
          ...
          successCount++;
          logger.info(`[SLA] Successfully unassigned request ${request.requestNumber}`);
        } catch (error) {
          logger.error(`[SLA] Failed to unassign request ${request.requestNumber}:`, error);
        }
      }

      return successCount;
    } catch (error) {
      logger.error('[SLA] Error in checkAndUnassignExpired:', error);
      throw error;
    }
  }
  ...
  private async unassignRequest(
    ...
    await db.transaction(async (tx) => {
      ...
      await tx.insert(requestAssignments).values({
        ...
        assignedAt: typeof request.assignedAt === 'string' 
    ...
    try {
      ...
      if (recipients.length > 0) {
        ...
        logger.warn(`[SLA] No notification recipients configured for request ${request.requestNumber}`);
      }
    } catch (emailError) {
      // Log email errors but don't fail the entire operation
      logger.error(`[SLA] Failed to send timeout email for ${request.requestNumber}:`, emailError);
    }
  }
}
